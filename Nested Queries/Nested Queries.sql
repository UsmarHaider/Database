...
Select Username
from users
where users.UserName not in (select username from Tweets)

select Users.UserName from Users
except (select Tweets.UserName from Tweets)

Select distinct UserName
from Tweets
where not Text like '%#Census%'
...
Select Username
from users as u
where not exists(select FollowedUserName from Following where u.UserName=FollowedUserName)

select UserInterests.InterestId, count(*) 
from  UserInterests
...
Select Country, count(*) as NoOfTweets
from Users join Tweets 
on Users.UserName = Tweets.UserName
group by Country

select Tweets .UserName, count(*) AS c
from  Tweets 
...
from Following
where FollowerUserName in (select Users.Username from Users where Users.Country = 'Pakistan')

select UserInterests.InterestID, Interests.Description,count(*) as cou from Interests, UserInterests
where UserInterests.InterestID =Interests.InterestID
Group by UserInterests.InterestID,Interests.[Description]
